...
func resourceStorage_Cloud_Azure() *schema.Resource {
    return &schema.Resource{
        Create: resourceCreateStorage_Cloud_Azure,
        Read:   resourceReadStorage_Cloud_Azure,
        Update: resourceUpdateStorage_Cloud_Azure,
        Delete: resourceDeleteStorage_Cloud_Azure,
        ...
        Schema: map[string]*schema.Schema{
            ...
            "cloudtype": {
                Type:        schema.TypeString,
                Required:    true,
                Description: "Name of cloud vendor [Microsoft Azure Storage]",
            },
            "authentication": {
                Type:        schema.TypeString,
                Required:    true,
...
func resourceCreateStorage_Cloud_Azure(d *schema.ResourceData, m interface{}) error {
    //API: (POST) /V4/Storage/Cloud
    var response_id = strconv.Itoa(0)
    var t_mediaagent *handler.MsgIdName
    if val, ok := d.GetOk("mediaagent"); ok {
        t_mediaagent = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
    }
    var t_name *string
    ...
    if val, ok := d.GetOk("credentials"); ok {
        t_credentials = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
    }
    ...
    if val, ok := d.GetOk("deduplicationdblocation"); ok {
        t_deduplicationdblocation = build_storage_cloud_azure_msgdedupepathset_array(d, val.(*schema.Set).List())
    }
    var t_usededuplication *bool
    if val, ok := d.GetOk("usededuplication"); ok {
        t_usededuplication = handler.ToBooleanValue(val, false)
    }
    var req = handler.MsgCreateCloudStorageAzureRequest{MediaAgent:t_mediaagent, Name:t_name, Container:t_container, StorageClass:t_storageclass, ServiceHost:t_servicehost, AccountName:t_accountname, Credentials:t_credentials, CloudType:t_cloudtype, Authentication:t_authentication, DeduplicationDBLocation:t_deduplicationdblocation, UseDeduplication:t_usededuplication}
    resp, err := handler.CvCreateCloudStorageAzure(req)
    if err != nil {
        return fmt.Errorf("operation [CreateCloudStorageAzure] failed, Error %s", err)
    }
    if resp.Id != nil {
        response_id = strconv.Itoa(*resp.Id)
    }
    if response_id == "0" {
        return fmt.Errorf("operation [CreateCloudStorageAzure] failed")
    } else {
        d.SetId(response_id)
        return resourceCreateUpdateStorage_Cloud_Azure(d, m)
    }
}

func resourceReadStorage_Cloud_Azure(d *schema.ResourceData, m interface{}) error {
    ...
    if rtn, ok := serialize_storage_cloud_azure_msgsecurityassocset_array(d, resp.Security); ok {
        d.Set("security", rtn)
    } else {
        d.Set("security", make([]map[string]interface{}, 0))
    }
    if rtn, ok := serialize_storage_cloud_azure_msgencryption(d, resp.Encryption); ok {
...
func resourceUpdateStorage_Cloud_Azure(d *schema.ResourceData, m interface{}) error {
    //API: (PUT) /V4/Storage/Cloud/{cloudStorageId}
    var t_security []handler.MsgUpdateSecurityAssocSet
    if d.HasChange("security") {
        val := d.Get("security")
        t_security = build_storage_cloud_azure_msgupdatesecurityassocset_array(d, val.(*schema.Set).List())
    }
    ...
    if d.HasChange("encryption") {
        ...
        t_encryption = build_storage_cloud_azure_msgencryption(d, val.([]interface{}))
    }
    ...
    return resourceReadStorage_Cloud_Azure(d, m)
}

func resourceCreateUpdateStorage_Cloud_Azure(d *schema.ResourceData, m interface{}) error {
    //API: (PUT) /V4/Storage/Cloud/{cloudStorageId}
...
func resourceDeleteStorage_Cloud_Azure(d *schema.ResourceData, m interface{}) error {
...
func build_storage_cloud_azure_msgencryption(d *schema.ResourceData, r []interface{}) *handler.MsgEncryption {
    if len(r) > 0 && r[0] != nil {
        ...
        if val, ok := tmp["keyprovider"]; ok {
            t_keyprovider = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
        }
...
func build_storage_cloud_azure_msgidname(d *schema.ResourceData, r []interface{}) *handler.MsgIdName {
    if len(r) > 0 && r[0] != nil {
        tmp := r[0].(map[string]interface{})
        var t_name *string
...
func build_storage_cloud_azure_msgupdatesecurityassocset_array(d *schema.ResourceData, r []interface{}) []handler.MsgUpdateSecurityAssocSet {
    if r != nil {
        ...
        for a, iter_a := range r {
            ...
            if val, ok := raw_a["role"]; ok {
                t_role = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
            }
            var t_user *handler.MsgIdName
            if val, ok := raw_a["user"]; ok {
                t_user = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
            }
            var t_usergroup *handler.MsgIdName
            if val, ok := raw_a["usergroup"]; ok {
                t_usergroup = build_storage_cloud_azure_msgidname(d, val.([]interface{}))
            }
...
func build_storage_cloud_azure_msgdedupepathset_array(d *schema.ResourceData, r []interface{}) []handler.MsgDedupePathSet {
    if r != nil {
...
func serialize_storage_cloud_azure_msgencryption(d *schema.ResourceData, data *handler.MsgEncryption) ([]map[string]interface{}, bool) {
    //MsgEncryption
    //MsgEncryption
    if data == nil {
        return nil, false
    ...
    if rtn, ok := serialize_storage_cloud_azure_msgidname(d, data.KeyProvider); ok {
...
func serialize_storage_cloud_azure_msgidname(d *schema.ResourceData, data *handler.MsgIdName) ([]map[string]interface{}, bool) {
...
func serialize_storage_cloud_azure_msgsecurityassocset_array(d *schema.ResourceData, data []handler.MsgSecurityAssocSet) ([]map[string]interface{}, bool) {
    ...
    for i := range data {
        ...
        added := false
        if rtn, ok := serialize_storage_cloud_azure_msgidname(d, data[i].Role); ok {
            tmp["role"] = rtn
            added = true
        }
        if rtn, ok := serialize_storage_cloud_azure_msgidname(d, data[i].User); ok {
            tmp["user"] = rtn
            added = true
        }
        if rtn, ok := serialize_storage_cloud_azure_msgidname(d, data[i].UserGroup); ok {
            tmp["usergroup"] = rtn
            added = true
        }
        if added {
