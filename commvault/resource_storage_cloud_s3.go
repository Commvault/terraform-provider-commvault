	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"cloudtype": {
				...
				Optional:    true,
				Computed:    true,
				Description: "Cloud storage vendor type [Amazon S3, Microsoft Azure, Google Cloud Storage, OCI Object Storage, Alibaba Cloud Object Storage Service, S3 Compatible Storage, ...]",
			},
			"authentication": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Authentication type for the cloud storage server. Required for Amazon S3. [Access and secret keys, AWS IAM role policy, AWS STS assume role, AWS STS assume role with IAM role policy]",
			},
...
func resourceCreateStorage_Cloud_S3(d *schema.ResourceData, m interface{}) error {
	...
	// Amazon S3-specific fields are not accepted by the API for other cloud types
	if t_cloudtype != nil && *t_cloudtype != "Amazon S3" {
		t_storageclass = nil
		t_authentication = nil
		t_arnrole = nil
	}
	...
	if response_id == "0" {
		if resp.ErrorMessage != nil && *resp.ErrorMessage != "" {
			return fmt.Errorf("operation [CreateCloudStorageS3] failed, errorCode: %d, errorMessage: %s", func() int {
