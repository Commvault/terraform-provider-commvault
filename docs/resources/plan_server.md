...
## Important: Backup Destination Approaches

There are **two ways** to associate backup destinations with a plan, each with different trade-offs:

### Approach 1: Inline `backupdestinations` (Recommended)

Create destinations **directly within** the plan resource. This prevents the Commvault API from auto-creating temporary plans and is the **recommended approach**:
...
**Pros:**
- Atomic creation: plan and destinations created together
- No temporary plans auto-generated by Commvault
- Clean, cohesive configuration
- Easier to manage lifecycle

### Approach 2: Separate `commvault_plan_backupdestination` Resources

Create destinations as standalone resources and reference them via `backupdestinationids`. **Use this only if you need to:**
- Reuse the same destination in multiple plans
- Manage destination lifecycle independently
- Create destinations in a separate Terraform apply

```hcl
resource "commvault_plan_backupdestination" "primary" {
...
resource "commvault_plan_server" "my_plan" {
  planname              = "MyBackupPlan"
  backupdestinationids = [commvault_plan_backupdestination.primary.id]
  # ... other plan settings
}
```

**Cons of Approach 2:**
- Commvault auto-creates a temporary plan (`PlanBkpDst_<UUID>_Region_0_Plan`) when the destination is first created
- This temporary plan permanently associates with the destination
- Once used in another plan, the destination cannot be reassociated
- Can lead to "already associated with a plan" errors if workflows overlap

**Recommendation:** Use Approach 1 (inline) unless you have a specific requirement for shared, independently-managed destinations.
